...
<tr>
    <td colspan="2"><p class="texto_bobo"><?=$Gen->limpiarCaracteresEspeciales($dbm->axm_texto)?></p></td>
  </tr>
  </tr>
  <tr>
...
<input type="hidden" name="tablaoriginal" value="<?=$tablaoriginal?>">
    ...
    <td width="334"><?php if($insertar==1){?><a class="ingresar" title="Ingresar Nuevo Registro" href="n.php?n=3&mio=32&nombretabla=<?=$tablaoriginal?>&v_id=<?=$db->id?>&id=<?=$dbaxmid?>&descripcion=<?=$Descripcion?>&campousu=<?=$dbm->axm_usu_id?>&campofechahora=<?=$dbm->axm_fechahora?>&submenu=<?=$dbm->axm_submenu?>&camposub=<?=$dbm->axm_camposub?>&campoequipo=<?=$dbm->axm_equipo?>">&nbsp;&nbsp;&nbsp;NUEVO REGISTRO</a><?php } ?></td>
  </tr>
</table> 
...
<tr><td colspan="2"></td></tr>
  ...
  <?php  
   ...
   else 
   {
	...
	elseif($tablaoriginal=="tramite_tipo")
	{
		 ...
		 if($_POST['busqueda']!='' && $tablaoriginal=="tramite_tipo")
		{ 
  ...
  <tr class="<?=$estilo?>">
    ...
    <td width="24" class="txt_espacio"><?php if($editar==1){?><a href="n.php?n=3&mio=31&tablaoriginal=<?=$tablaoriginal?>&v_id=<?=$db->id?>&id=<?=$dbaxmid?>&descripcion=<?=$descripcion?>&campousu=<?=$dbm->axm_usu_id?>&campofechahora=<?=$dbm->axm_fechahora?>&submenu=<?=$dbm->axm_submenu?>&camposub=<?=$dbm->axm_camposub?>&campoequipo=<?=$campoequipo?>"><img src="tabbed_pages/editar.png" title="Editar" alt="Editar" width="20" height="20" /></a><?php } ?></td>
    <td width="25" class="txt_espacio"><?php if($borrarlogico==1){?><img title="Eliminar" alt="Eliminar" src="tabbed_pages/eliminar.png" width="20" height="20" onclick="eliminar('<?=$campo_id?>','<?=$tablaoriginal?>','<?=$db->id?>','<?=$dbm->axm_estado?>','<?=$dbm->axm_id?>','<?=$dbm->axm_submenu?>','<?=$dbm->axm_camposub?>','');"/><?php } ?></td>
  </tr>
